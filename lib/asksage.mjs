...
// ─────────────────────────────────────────────────────────────────────────
import { buildMessage, buildMultiMessage, buildChangesMessage } from "./template.mjs";
...
export async function summarizeDocuments(documents, previous) {
  ...
  return { summary: await queryComplete(message), chars };
}

/**
 * Change-review flow (does NOT summarize). Given this week's already-extracted
 * document(s) and last week's report, return ONLY a short block of suggested
 * changes — chiefly what this week appears to be missing versus last week. This
 * week's text is reproduced verbatim by the browser, never here.
 *
 * documents: non-empty array of { filename, text }. previous: last week's
 * { filename, text } (optional). With no previous there is no baseline, so we
 * short-circuit with a first-report notice and never call the model.
 *
 * Uses plain query() (not queryComplete): the block is a handful of bullets, far
 * under the completion cap, so the [END OF REPORT] continuation protocol — which
 * would also pollute a short block — is unnecessary.
 */
export async function analyzeChanges(documents, previous) {
  if (!Array.isArray(documents) || documents.length === 0) {
    throw new Error("Nothing to compare — no documents were provided.");
  }
  for (const doc of documents) {
    if (typeof doc?.text !== "string" || !doc.text.trim()) {
      throw new Error("Every document needs non-empty text. Nothing was compared.");
    }
    const len = doc.text.trim().length;
    if (len < 30) {
      throw new Error(
  ...
  if (previous != null) {
    if (typeof previous.text !== "string" || !previous.text.trim()) {
      throw new Error("Last week's report has no text. Nothing was compared.");
    }
    const plen = previous.text.trim().length;
    if (plen < 30) {
      throw new Error(
  ...
  // Same total-size guard as the summarize path: this week's docs plus last
  // week's report must fit before anything is sent. Nothing is ever truncated.
  const chars =
    documents.reduce((n, doc) => n + doc.text.length, 0) + (previous ? previous.text.length : 0);
  if (chars > config.maxInputChars) {
    throw new Error(
      `Combined input is too long (${chars.toLocaleString()} characters, ` +
        `limit ${config.maxInputChars.toLocaleString()}). Compare fewer or smaller documents ` +
        `at a time — nothing was truncated or sent.`
    );
  }
  // No prior week → nothing to compare against. Deterministic notice, no AI call.
  if (previous == null) {
    return {
      changes:
        "**Suggested changes**\n\n- First report — no prior week to compare against. " +
        "This week's document is shown below.",
      chars,
    };
  }
  return { changes: await query(buildChangesMessage(documents, previous)), chars };
}
