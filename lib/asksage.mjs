...
// ─────────────────────────────────────────────────────────────────────────
import { buildMessage, buildMultiMessage } from "./template.mjs";
import { ensureCompleteness } from "./ledger.mjs";
import { reconcileAudit } from "./reconcile.mjs";
 ...
 */
export async function summarizeDocuments(documents, previous) {
  ...
  // Optional prior-week report: same floor, same never-summarize-garbage rule.
  if (previous != null) {
    if (typeof previous.text !== "string" || !previous.text.trim()) {
      throw new Error("Last week's report has no text. Nothing was summarized.");
    }
    const plen = previous.text.trim().length;
    if (plen < 30) {
      throw new Error(
        `Last week's report ("${previous.filename || "previous report"}") has only ${plen} characters — ` +
          `that isn't a report. Nothing was summarized.`
      );
    }
  }
  // The limit guards the TOTAL prompt size — five medium documents (plus last
  // week's report riding along) add up just like a single 500-page PDF.
  const chars =
    documents.reduce((n, doc) => n + doc.text.length, 0) + (previous ? previous.text.length : 0);
  if (chars > config.maxInputChars) {
    throw new Error(
      `Combined input is too long (${chars.toLocaleString()} characters across ${documents.length} ` +
        `document${documents.length === 1 ? "" : "s"}${previous ? " plus last week's report" : ""}, ` +
        `limit ${config.maxInputChars.toLocaleString()}). ` +
        `Summarize fewer or smaller documents at a time — nothing was truncated or sent.`
    );
  }
  const message =
    documents.length === 1
      ? buildMessage(documents[0].text, previous)
      : buildMultiMessage(documents, previous);
  let summary = normalizeTableBlocks(await queryComplete(message));

  // Completeness pass (lib/ledger.mjs): extract a fact ledger from the
  // documents, verify every fact appears in the draft, splice in the
  // missing ones as code-applied patches. Fails OPEN: any error here
  // returns the untouched draft rather than losing it. Disable with
  // ASKSAGE_COMPLETENESS=off.
  let completeness = null;
  if ((process.env.ASKSAGE_COMPLETENESS || "on") !== "off") {
    try {
      const r = await ensureCompleteness(query, documents, summary);
      summary = r.report;
      completeness = r.stats;
    } catch (err) {
      completeness = { error: "completeness pass failed: " + err.message };
    }
  }

  // Staleness audit (lib/reconcile.mjs): when a prior report rides along,
  // no number that exists only in LAST week's report may ship outside an
  // explicit "(was …)" comparison. One bounded repair round; leftovers are
  // reported, never hidden. Fails OPEN like the completeness pass above.
  // Disable with ASKSAGE_RECONCILE=off.
  if (previous && (process.env.ASKSAGE_RECONCILE || "on") !== "off") {
    try {
      const r = await reconcileAudit(query, documents, previous, summary);
      summary = r.report;
      completeness = completeness || {};
      completeness.reconcile = r.stats;
    } catch (err) {
      completeness = completeness || {};
      completeness.reconcile = { error: "staleness check failed: " + err.message };
    }
  }
  // Final shape pass: the completeness/reconcile patch rounds splice lines
  // by anchor and can land one inside a table block.
  summary = normalizeTableBlocks(summary);
  return { summary, chars, completeness };
}
...
export async function query(message) {
  ...
  const data = await callJson("/server/query", {
    ...
    body: {
      ...
      temperature: 0.0,
      // Raise the output ceiling: with 7+ documents the full report needs
      // more than Ask Sage's default completion budget (~10K tokens), and
      // the overflow was TRUNCATED mid-sentence, amputating the final
      // sections (Anomaly Status table, Progress to Closure). Verified
      // 22 Jul 2026: 7-doc run cut at ~37K chars; 5-doc runs fit.
      max_tokens: 32000,
  ...
  const text = pick(data, ["message", "response", "answer", "content", "result", "text"], "the model's answer");
  assertNotQuotaMessage(text);
  return text;
}

/** Ask Sage reports monthly-quota exhaustion as a NORMAL completion
 *  (HTTP 200 whose body text is an apology string). Without this guard
 *  that string ships as the "summary" under a green Done — observed live
 *  23 Jul 2026 (2,800,956 of 2,800,000 tokens). Convert it to the error
 *  it actually is. */
function assertNotQuotaMessage(text) {
  if (String(text).length < 600 && /above your monthly token limit/i.test(text)) {
    throw new Error(
      "Ask Sage says this account is over its monthly token limit, so nothing was " +
        "summarized. Contact your Ask Sage administrator to raise the limit or wait " +
        "for the monthly reset."
    );
  }
}

// ── Truncation self-healing ────────────────────────────────────────────────
// Ask Sage hard-caps completion length (~36K chars observed) and IGNORES a
// max_tokens override. With 7+ documents the full report exceeds the cap and
// arrives cut off mid-sentence, silently amputating the final sections (the
// Anomaly Status table and Progress to Closure). The prompt (template.mjs)
// asks the model to end with a literal [END OF REPORT] line; when that line
// is missing, the report was truncated, and we ask the model to continue
// from where it stopped, up to two times, then stitch the pieces.
const END_SENTINEL = "[END OF REPORT]";

function stripSentinel(s) {
  return String(s).replace(/\s*\[END OF REPORT\]\s*$/i, "").trimEnd();
}

/**
 * A prose line stranded BETWEEN two table rows breaks the whole table's
 * markdown rendering (observed live 23 Jul 2026: a watch-item's detail
 * sentence emitted mid-table). Relocate any non-pipe line that sits between
 * pipe rows to immediately after the table, preserving order. Deterministic
 * and shape-only — no text is altered or dropped. Patch passes can also
 * insert lines into tables, so this runs both after generation and at the
 * end of the pipeline (see summarizeDocuments).
 */
export function normalizeTableBlocks(s) {
  ...
  while (i < lines.length) {
    ...
    // Inside a table block: keep rows; a single non-blank prose line whose
    // very next line is another row is stranded — hold it until the block
    // ends. A blank line or two consecutive prose lines end the table.
    const displaced = [];
    while (i < lines.length) {
      if (isRow(lines[i])) {
        out.push(lines[i++]);
      ...
      if (lines[i].trim() && isRow(lines[i + 1] || "")) {
        displaced.push(lines[i++]);
        continue;
      }
      break;
    }
...
/** Drop a trailing cut-off fragment: keep lines up to the last one that ends
 *  cleanly (period, table pipe, closing bracket, or a blank line). */
function trimToCompleteLine(s) {
...
export async function queryComplete(message) {
  let report = await query(message);
  for (let attempt = 0; attempt < 2 && !report.includes(END_SENTINEL); attempt++) {
    const partial = trimToCompleteLine(report);
    const continuation = await query(
      message +
        "\n\nThe report below was cut off before it finished. Continue it from" +
        " exactly where it stops: output ONLY the continuation, never repeat" +
        " anything already written, complete every remaining section of the" +
        " required structure in order, and end with the line " + END_SENTINEL + "\n\n" +
        "<partial_report>\n" + partial + "\n</partial_report>"
    );
    report = partial + "\n" + continuation;
  }
  return stripSentinel(report);
}
