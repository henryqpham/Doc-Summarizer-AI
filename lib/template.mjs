...
/**
 * Today's date, for the "Week Ending" header ONLY — the one deliberate
 ...
 * document." Every other fact must still come from the material; a period
 * stated in the documents always wins over the injected date.
 */
function dateRule() {
  ...
  return [
    `- Today's date is ${today}. As the ONLY exception to the use-only-the-`,
    '  material rule above, you may use it to fill the "Week Ending" header',
    "  when the documents do not state the reporting period themselves — a",
    "  period stated in the documents always wins. Never use today's date",
    "  for any other fact, number, or delta.",
  ];
}
...
function priorRules(hasPrevious) {
  ...
  return [
    "- NO prior report was provided — this is the first report. Every Trend",
    '  cell must be ↔ with "(no prior report)" appended to that row\'s first',
    '  cell; "Key Changes Since Last Week" must state that this is the first',
    "  report with no prior week to compare; never use trajectory wording",
    '  that implies a previous week ("remains", "continues", "still") or',
    "  invent any comparison.",
  ];
}
